...
// System definitions for `arcane_starter-actions` contract
public class Actions : MonoBehaviour {
    ...
    // Call the `spawn_enemy` system with the specified Account and calldata
    // Returns the transaction hash. Use `WaitForTransaction` to wait for the transaction to be confirmed.
    public async Task<FieldElement> spawn_enemy(Account account, int pos_x, int pos_y, sbyte enemy_type) {
        List<dojo.FieldElement> calldata = new List<dojo.FieldElement>();
        calldata.Add(new FieldElement(pos_x).Inner);
		calldata.Add(new FieldElement(pos_y).Inner);
		calldata.Add(new FieldElement(enemy_type).Inner);

        return await account.ExecuteRaw(new dojo.Call[] {
            new dojo.Call{
                to = new FieldElement(contractAddress).Inner,
                selector = "spawn_enemy",
                calldata = calldata.ToArray()
            }
        });
    }
